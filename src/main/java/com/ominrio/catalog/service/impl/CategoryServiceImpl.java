		...
		if (attributeList != null && !attributeList.isEmpty()) {
			...
			if (category.isPresent()) {
				...
				for (Attribute attribute : attributeList) {
					try {
						Attribute findAllById = null;
						Optional<Attribute> findById = attributeRepository.findById(attribute.getId());
						findAllById = findById.get();
						findAllById.setCategory(category.get());

						if (findAllById != null) {
							Attribute save = attributeRepository.save(findAllById);
